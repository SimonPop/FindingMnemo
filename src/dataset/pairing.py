...
class PairingMaker():
    def __init__(self):
        ...
        self.columns = ['chinese_hanzi', 'chinese_pinyin', 'chinese_phonetic', 'english_word','english_phonetic','distance']
        self.best_pairs_path = self.path / '../../data/best_pairs.csv'
        self.worst_pairs_path = self.path / '../../data/worst_pairs.csv'
    ...
    def custom_distance(self, chinese_row, english_row):
        ...
        dolgo_dst = self.dst.fast_levenshtein_distance(chinese_row['dolgo'], english_row['dolgo']) / max(len(chinese_row['dolgo']), len(english_row['dolgo']))
        ipa_dst = self.dst.fast_levenshtein_distance(chinese_row['ipa'], english_row['ipa']) / max(len(chinese_row['ipa']), len(english_row['ipa']))
        return (dolgo_dst + ipa_dst) / 2 
    ...
    def find_pair_from_chinese(self, chinese_row, english_df: pd.DataFrame):
        ...
        distances = [self.custom_distance(chinese_row, english_row) for _, english_row in tqdm(english_df.iterrows(), desc=desc.format(chinese_row['pinyin']))]
    ...
    def load_pairs(self) -> List[pd.DataFrame]:
        ...
        try:
            ...
            col_names = ['chinese_hanzi', 'chinese_pinyin', 'chinese_phonetic', 'english_word','english_phonetic','distance']
    ...
    def load_english_data(self):
        ...
        english_corpus = pd.read_csv(self.path / '../../data/en.csv')
        return english_corpus[english_corpus['valid_ipa'] == True]

    def load_chinese_data(self, pairs):
        print('Loading chinese corpus.')
        hsk_df = pd.read_csv(self.path / '../../data/zh.csv')
