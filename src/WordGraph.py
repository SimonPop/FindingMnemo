...
class WordGraph():
    ...
    def __init__(self, corpus: str, state_size: int = 5, distance: str = 'dolgo_prime_distance_div_maxlen'):
        self.corpus = corpus
        self.cutoff = 20
        self.chain = None
    ...
    def find_closest_word(self, word:str) -> str:
        ...
        distances = [self.dst_function(phonetic_word, w2) for w2 in tqdm(self.phonetic_vocab)]
    ...
    def find_path(self, words: List[str]) -> List[str]:
        ...
        for a in all_A:
            for b in all_B:
                path_to_A = nx.shortest_path(self.graph, source=('___BEGIN__', '___BEGIN__'), target=a)
