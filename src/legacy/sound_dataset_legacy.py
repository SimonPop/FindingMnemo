...
class SoundDataset(Dataset):
    ...
    def find_best_pairs(
        self, english_vocabulary: List[str], chinese_vocabulary: List[str]
    ):
        ...
        for phonetic_word in english_vocabulary:
            distances = [
                self.dst.dolgo_prime_distance(phonetic_word, w2)
                for w2 in tqdm(
                    chinese_vocabulary,
                    desc="Finding closest word for {}".format(phonetic_word),
                )
            ]
            best_matches.append(
                (
                    phonetic_word,
                    chinese_vocabulary[np.argmin(distances)],
                    np.argmin(distances),
                )
            )
            worst_matches.append(
                (
                    phonetic_word,
                    chinese_vocabulary[np.argmax(distances)],
                    np.argmax(distances),
                )
            )

        return {"positive": best_matches, "negative": worst_matches}
