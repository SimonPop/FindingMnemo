...
def create_dataset(G, size: int = 1000) -> LinkPairDataset:
    ...
    mask = np.array(["train"]*third + ["val"]*third + ["test"]*(len(enriched_nodes) - 2*third))
    ...
    for mode in ['train', 'val', 'test']:
        ...
        positive_pairs = [] # [(x[0], x[1]) for x in positive_pairs[np.random.randint(0, len(positive_pairs), size//2)]]
        negative_pairs = np.argwhere(H == 0)
        negative_pairs = [(x[0], x[1]) for x in negative_pairs[np.random.randint(0, len(negative_pairs), size//2)]]
        ...
        for a, b in positive_pairs:
            pair_features = {
                ...
                'common_neighbors': len(list(nx.common_neighbors(G, enriched_nodes[a], enriched_nodes[b]))),
                "common_categories": len(categories[a].intersection(categories[b])),
                "degree_sum": degrees[enriched_nodes[a]] + degrees[enriched_nodes[b]],
                "degree_diff": abs(degrees[enriched_nodes[a]] - degrees[enriched_nodes[b]]),
                "connected": enriched_nodes[a] in nx.neighbors(G, enriched_nodes[b]) or enriched_nodes[b] in nx.neighbors(G, enriched_nodes[a])
            }
