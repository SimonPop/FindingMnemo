...
@click.command()
@click.option('--pair-margin', default=0.2, type=float, help='Minimum distance between a "good" and "bad" pair for the same word.')
@click.option('--pair-saving-frequency', default=10, type=int, help='Saves new pair every x iterations into the file.')
def process(pair_margin: float, pair_saving_frequency: int):
    ...
    pair_maker.make_pairs(english_dataset_path, mandarin_dataset_path, output_directory=data_path, saving_frequency=pair_saving_frequency)
