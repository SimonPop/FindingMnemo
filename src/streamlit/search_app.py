...
def display_candidates(response, translation, pinyin):
    ...
    for index, (match, emoji) in enumerate(zip(matches, emojis)):
        generation = requests.get(
            url=f"http://fastapi:8000/generate/{translation}/{match}/"
        ).json()
        st.write(f"{emoji} - {match} - {generation}")
    st.info(
        ":tropical_fish: - Definitely Mnemo / :fish: - Not exactly Mnemo / :blowfish: - Barely Mnemo"
    )
...
if chinese_word:
    ...
    translation = (
        d.lookup(chinese_word).definition_entries[0].definitions[0].split("(")[0]
    )
